...
void round_coord(incoord, outcoord, prec)
...
void aberrate(epoch, vec, from_std)
...

{
	...
	if (PRINT_TO_SCREEN) printf("Earth XYZ (AU, J2000 frame): %7.4f  %7.4f  %7.4f\n",xearth,yearth,zearth);
	...
	*ddec = (decpar - dec2000) * 3600.;
	...
	if (PRINT_TO_SCREEN) printf("Parallax factors (arcsec, J2000): dRA = %7.4f, ddec = %7.4f\n",*dra,*ddec);
	...
	*aberdec = (decaber - dec2000) * 3600.;
	...
	if (PRINT_TO_SCREEN) printf("Annual aberration (arcsec): dra = %8.3f, ddec = %8.3f\n",*aberra,*aberdec);
